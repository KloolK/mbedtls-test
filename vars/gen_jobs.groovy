...
private Map<String, Callable<Void>> instrumented_node_job(String job_name, String node_label, Callable<Void> body) {
    return job(job_name) {
...
Map<String, Callable<Void>> gen_simple_windows_jobs(String label, String script) {
    return instrumented_node_job(label, 'windows') {
        try {
            dir('src') {
                deleteDir()
                ...
                timeout(time: common.perJobTimeout.time,
                    ...
                    analysis.record_inner_timestamps(label) {
                        bat script
                    }
                }
            }
...
'''
    ...
    return instrumented_node_job(job_name, node_label) {
        try {
            deleteDir()
            if (use_docker) {
...
"""
            ...
            timeout(time: common.perJobTimeout.time,
                ...
                try {
                    if (use_docker) {
                        analysis.record_inner_timestamps(job_name) {
...
def gen_windows_testing_job(build, label_prefix='') {
    ...
    return instrumented_node_job(job_name, 'windows') {
        try {
            dir("src") {
                deleteDir()
                checkout_repo.checkout_repo()
...
def gen_abi_api_checking_job(platform) {
    ...
    return instrumented_node_job(job_name, 'container-host') {
        try {
            deleteDir()
            common.get_docker_image(platform)
            dir('src') {
...
def gen_mbed_os_example_job(repo, branch, example, compiler, platform, raas) {
    ...
    return instrumented_node_job(job_name, compiler) {
        try {
            deleteDir()
...
def gen_coverity_push_jobs() {
    ...
    if (env.MBED_TLS_BRANCH == "development") {
        jobs << instrumented_node_job(job_name, 'container-host') {
            try {
                dir("src") {
                    deleteDir()
                    checkout_repo.checkout_repo()
...
"""
                        ...
                        analysis.record_inner_timestamps(platform) {
                            sh """\
